...
# Объединяем части на Railway сразу в корень /data (Volume, сохраняется между перезапусками)
echo ""
echo "🔗 Объединение частей на Railway в корень /data..."
(cd "$PROJECT_DIR" && railway run --service "$SERVICE_NAME" bash -c "
# Используем корень /data (Volume, сохраняется между перезапусками)
if [ -d /data ]; then
    TARGET_DIR=/data
    echo '✅ Используем корень /data (Volume, сохраняется между перезапусками)'
else
    TARGET_DIR=/tmp
