# 🚀 Быстрая миграция данных в Railway Volume

## Шаг 1: Создание Railway Volume

1. Откройте [Railway Dashboard](https://railway.app)
2. Выберите ваш проект → **Settings** → **Volumes**
3. Нажмите **"Create Volume"**:
   - **Name**: `data-volume`
   - **Mount Path**: `/data`
   - **Size**: `10 GB` (или больше, если данных много)

## Шаг 2: Автоматическая миграция (рекомендуется)

Самый простой способ - использовать автоматический скрипт:

```bash
cd /Users/dmitryalexeev/sta-black-master

# Установите Railway CLI (если еще не установлен)
brew install railway

# Авторизуйтесь
railway login

# Подключитесь к проекту
railway link

# Запустите автоматическую миграцию
export RAILWAY_SERVICE=web  # Или имя вашего сервиса
./scripts/upload_to_railway.sh
```

Скрипт автоматически:
1. Создаст архив данных
2. Загрузит его на Railway
3. Распакует в `/data`
4. Проверит результат

## Шаг 3: Ручная миграция (альтернатива)

Если автоматический скрипт не работает, используйте ручной способ:

### Шаг 3.1: Подготовка архива

```bash
cd /Users/dmitryalexeev/sta-black-master
./scripts/migrate_to_railway.sh
```

Скрипт создаст архив с данными в `/tmp/data-backup-*.tar.gz`

### Шаг 3.2: Загрузка данных на Railway

### Вариант A: Через Railway CLI

```bash
# 1. Установите Railway CLI (если еще не установлен)
brew install railway

# 2. Авторизуйтесь
railway login

# 3. Подключитесь к проекту
railway link

# 4. Загрузите архив на Railway
railway run --service <your-service-name> bash

# Внутри контейнера Railway:
cd /tmp
# Загрузите архив через base64 или другой способ
# Или используйте Railway Dashboard → Deployments → Upload Files
```

### Вариант B: Через скрипт upload_via_base64.sh

Если архив уже создан, используйте специальный скрипт:

```bash
./scripts/upload_via_base64.sh /tmp/data-backup-*.tar.gz
```

Скрипт автоматически загрузит архив через base64.

## Шаг 4: Распаковка данных

```bash
# Подключитесь к Railway
railway shell

# Или через railway run
railway run --service <service-name> --command "cd /data && tar -xzf /tmp/data-backup-*.tar.gz"
```

## Шаг 5: Проверка

```bash
# Проверьте количество файлов
railway run --service <service-name> --command "find /data/img -type f | wc -l"

# Проверьте размер данных
railway run --service <service-name> --command "du -sh /data/*"

# Проверьте доступность файлов
railway run --service <service-name> --command "ls -la /data/img | head -10"
```

## Что мигрируется

- ✅ `/data/img/` - фотографии пользователей
- ✅ `/data/giftimg/` - изображения подарков  
- ✅ `/data/log/` - логи приложения

**НЕ мигрируется:**
- ❌ `tinder.db` - используется PostgreSQL
- ❌ `gift.bd` - используется PostgreSQL

## Альтернативный способ: Python скрипт

Если у вас есть прямой доступ к Railway серверу:

```bash
# Загрузите скрипт на Railway
railway run --service <service-name> --command "cat > /tmp/migrate.py" < python_backend/migrate_data_to_railway.py

# Запустите миграцию
railway run --service <service-name> --command "python /tmp/migrate.py --source /tmp/data --target /data"
```

## Важные замечания

1. **Размер Volume**: Убедитесь, что размер достаточен для всех данных
2. **Время миграции**: Может занять время в зависимости от объема
3. **Бэкап**: Создайте резервную копию перед миграцией
4. **Проверка**: После миграции проверьте, что все файлы доступны

## Устранение проблем

### Volume не монтируется
- Проверьте, что Volume создан в том же регионе
- Убедитесь, что Mount Path = `/data`
- Перезапустите сервис

### Недостаточно места
- Увеличьте размер Volume через Dashboard
- Или удалите ненужные файлы

### Ошибки доступа
- Проверьте права доступа к `/data`
- Убедитесь, что приложение запущено с правильными правами

## После миграции

1. ✅ Проверьте работу приложения
2. ✅ Убедитесь, что фотографии загружаются
3. ✅ Проверьте логи в `/data/log`
4. ✅ Удалите локальную директорию (после проверки)

