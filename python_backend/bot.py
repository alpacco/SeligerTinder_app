...
def get_start_keyboard():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎁 Промокод на PRO", callback_data="promo_code_info")],
        [InlineKeyboardButton("Меню", callback_data="show_menu")]
    ])
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "promo_code_info":
        promo_message = (
            "🎁 Мы добавили возможность активировать PRO-аккаунт за звёзды Telegram ⭐\n\n"
            "Вот что даёт PRO:\n\n"
            "• видно, сколько лайков вы поставили и сколько получили\n"
            "• при свайпе появляется плашка «Match 100%» — если вас уже лайкнули\n"
            "• Суперлайки — даже если мэтча не было, вашу анкету увидят ещё раз\n"
            "• возможность вернуться к анкете, если случайно свайпнули не туда (да, мы все там были 😅)\n"
            "• видно, когда пользователь был онлайн в последний раз\n"
            "• и да, приятный обновлённый дизайн — в подарок 😌\n\n"
            "А ещё небольшой инсайд 👀\n"
            "Наша команда сделала ещё одно приложение — для аренды и сдачи парковочных мест:\n"
            "👉 https://t.me/SecretParking_bot/spb\n\n"
            "Если у вас иногда простаивает парковка — её можно легко превратить в дополнительный доход.\n"
            "А если нужно куда-то встать на время — быстро найти свободное место.\n\n"
            "🎁 В честь этого дарим вам PRO на 7 дней бесплатно\n\n"
            "Как получить:\n"
            "1️⃣ Зайдите в бот 👉 https://t.me/SecretParking_bot/spb и откройте приложение\n"
            "2️⃣ В боте Secret Tinder: Меню → Купить PRO → Ввести промокод\n"
            "3️⃣ Введите промокод SECRETPARKING\n\n"
            "Готово — PRO на 7 дней у вас в кармане 💫"
        )
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("⭐ Купить PRO", callback_data="buy_pro_menu")],
            [InlineKeyboardButton("Назад", callback_data="show_menu")]
...
def create_bot_application():
    ...
    try:
        ...
        async def text_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            # Обработка dev_message
            elif state == "waiting_for_dev_message":
                print(f"✅ [BOT] ========== СОСТОЯНИЕ ПОДТВЕРЖДЕНО, ОБРАБАТЫВАЕМ СООБЩЕНИЕ ==========")
            ...
            # Если состояние не соответствует ожидаемому, игнорируем сообщение
                ...
                print(f"⚠️ [BOT] text_message_handler: состояние не соответствует ожидаемому ({state}), игнорируем")
